...
use ic_cdk::{self, init, pre_upgrade, post_upgrade, query};
use serde::Deserialize;
use std::cell::RefCell;
use crate::ic_cdk::update;
...
#[query]
fn get_all(done_filter: Option<bool>) -> Vec<TodoItem> {
...
#[query]
fn get_by_id(id_filter: Option<u32>) -> Vec<TodoItem> {
    RUNTIME_STATE.with(|state| get_by_id_impl(id_filter, &state.borrow()))
}

fn get_by_id_impl(id_filter: Option<u32>, runtime_state: &RuntimeState) -> Vec<TodoItem> {
    runtime_state.data.items.iter().filter(|i| match id_filter {
        Some(id) => i.id == id,
        None => true,
    }).cloned().collect()
}
...
#[update]
fn delete_by_id(id: u32) -> bool {
    RUNTIME_STATE.with(|state| delete_by_id_impl(id, &mut state.borrow_mut()))
}

fn delete_by_id_impl(id: u32, runtime_state: &mut RuntimeState) -> bool {
    if let Some(pos) = runtime_state.data.items.iter().position(|i| i.id == id) {
        runtime_state.data.items.remove(pos);
        true
    } else {
        false
    }
